...
def run_sniper_audit():
    print("🎯 LANZANDO SNIPER EN ORO CONGELADO ($500 USD)")
    ...
    config = {
        ...
        'fee': 0.0000, # SIMULANDO SPREAD CERO PARA VER EL VOLUMEN
        'type': 'futures',
    ...
    res = backtest(config, [{'exchange': exchange, 'symbol': symbol, 'timeframe': '1m', 'strategy': 'ExnessSniper'}], 
    ...
    report = {
        'Estrategia': 'ExnessSniper_HFT',
        'ROI': f"{m.get('net_profit_percentage', 0):.2f}%",
        'Trades': len(trades),
        'WinRate': f"{m.get('win_rate', 0)*100:.2f}%"
    }
    
    pd.DataFrame([report]).to_csv('REPORTE_SNIPER.csv', index=False)
    print(f"\n[V] Auditoría finalizada. Trades ejecutados: {len(trades)}")
